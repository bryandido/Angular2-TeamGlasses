...
// Change URL depending on where you're hosting it.
  ...
  getByPassword(password:string){
    return this._http.get('http://10.250.190.152/teamglasses/teamglasses-api/api.php/user?filter[]=password,eq,'+password+'&transform=1').map(res => res.json());
  }
  createUser(user:any){
    ...
    console.log(json);
    return this._http.post('http://10.250.190.152/teamglasses/teamglasses-api/api.php/user/', user).map(res => res.json());
  }
  delete(id: string){
    return this._http.delete('http://10.250.190.152/teamglasses/teamglasses-api/api.php/user/' + id).map(res => res.json());
  }
}
